...
import 'package:snapchatClone/screens/home/stories/user_stories.dart';
import 'package:video_player/video_player.dart';

class StoriesChild extends StatefulWidget {
  final String theFilePrev;
  final String theUid;
  final int index;
  StoriesChild({this.theFilePrev, this.theUid, this.index});
  @override
  _StoriesChildState createState() => _StoriesChildState();
}

class _StoriesChildState extends State<StoriesChild> {
      ...
      return FutureBuilder(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.done ||
              ...
              return ClipRRect(borderRadius: BorderRadius.circular(100 / 2), child: VideoPlayer(_controller));
            } else {
              return Container(
  ...
  Widget build(BuildContext context) {
    return OpenContainer<String>(
        ...
        openBuilder: (_, closeContainer) =>
            UserStories(uid: widget.theUid, index: widget.index, onClose: closeContainer),
        onClosed: (res) {
          if (mounted) {
            setState(() {
              navigateFSString = res;
            });
          }
        },
        tappable: false,
