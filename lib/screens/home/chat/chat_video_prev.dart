...
class ChatVideoPrev extends StatefulWidget {
  final String imgPath;
  final String userName;
  final String chatRoomId;
  ChatVideoPrev({this.imgPath, this.userName, this.chatRoomId});

  @override
  _ChatVideoPrevState createState() => _ChatVideoPrevState();
}

class _ChatVideoPrevState extends State<ChatVideoPrev> {
  ...
  sendMessage(BuildContext context) async {
    final user = Provider.of<CustomUser>(context, listen: false);
    String filePath = 'chat/${DateTime.now()}.mp4';
    _uploadTask = _storage.ref().child(filePath).putFile(File(widget.imgPath));
    _taskSnapshot = await _uploadTask.onComplete;
    final String downloadUrl = await _taskSnapshot.ref.getDownloadURL();
    DocumentReference docRef =
